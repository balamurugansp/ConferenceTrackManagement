...
import com.conference.management.vo.Lunch;
import com.conference.management.vo.Networking;
import com.conference.management.vo.Slot;
import com.conference.management.vo.Talk;
import com.conference.management.vo.TalksCompare;
...
public class ConferenceManager {
	...
	public List<Talk> fetchTalksListFromSource(DataSourceEnum dataSourceEnum) throws UnsupportedSourceException {
		...
		// Not exactly a factory. This will create an instance of the required
		// SourceManager
		if (dataSourceEnum.equals(DataSourceEnum.FILE)) {
			FileSourceManager sourceManager = new FileSourceManager();
			try {
	...
	public Conference processAndScheduleTalks(List<Talk> talkList) {
		...
		while (0 != talkList.size()) {
			...
			Slot morningSlot = new Slot(Configuration.MORNING_SLOT_DURATION_MINUTES, Configuration.TRACK_START_TIME);
			fillSlot(morningSlot, talkList);

			// create and fill lunch slot.
			Slot lunchSlot = new Slot(Configuration.LUNCH_DURATION_MINUTES, Configuration.LUNCH_START_TIME);
			lunchSlot.addEvent(new Lunch());

			// create and fill afternoon slot.
			Slot afternoonSlot = new Slot(Configuration.AFTERNOON_SLOT_DURATION_MINUTES,
					Configuration.POST_LUNCH_SLOT_START_TIME);
			fillSlot(afternoonSlot, talkList);

			// create and fill networking slot.
			Slot networkingSlot = new Slot(Configuration.NETWORKING_DURATION_MINUTES,
					Configuration.NETWORKING_START_TIME);
